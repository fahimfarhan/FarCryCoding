...
public:
    ...
    bool dfs(const Node& u, const string& word, const int& depth, const int& maxDepth) {
      ...
      if(depth == maxDepth) {
        if(ch == '.') {
          bool areAllSiblingsEmpty = true;
          for(int i=0; i<SIZE; i++) {
            //if(parent!=nullptr) {
              if(u.mp[i] != nullptr) {
                areAllSiblingsEmpty = false;
                break;
              }
            //}
          }
          if(areAllSiblingsEmpty) {
            return false;
          }
          return true;
...
int main() {
  ...
  bool b5 = wordDictionary.search("a.");
  wordDictionary.addWord("bad");
  wordDictionary.addWord("dad");
  wordDictionary.addWord("mad");

  bool b0 = wordDictionary.search("pad"); // return False  
